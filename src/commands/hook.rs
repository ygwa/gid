...
fn install_local_hook() -> Result<()> {
    ...
    let hooks_dir = git.repo_path()
        .ok_or_else(|| anyhow::anyhow!("无法获取仓库路径"))?
        .join("hooks");
    
    fs::create_dir_all(&hooks_dir)
        .context("无法创建 hooks 目录")?;
...
fn install_global_hook() -> Result<()> {
    ...
    println!("  {}", hook_path.display().to_string().dimmed());
    println!();
    println!("已设置 {} = {}", "core.hooksPath".cyan(), hooks_dir.display());
...
fn uninstall_global_hook() -> Result<()> {
    ...
    let hook_path = home.join(".config").join("git").join("hooks").join("pre-commit");
...
fn show_status() -> Result<()> {
    ...
    if git.is_in_repo() {
        let local_hook = git.repo_path()
            .map(|p| p.join("hooks").join("pre-commit"));
        ...
        if let Some(hook_path) = local_hook {
            if hook_path.exists() {
                ...
                if is_gid {
                    ...
                    println!("  {} 本地 hook: {}", "!".yellow(), "已存在 (非 gid)".yellow());
                }
                println!("    {}", hook_path.display().to_string().dimmed());
            } else {
        ...
        println!("  {} 本地 hook: {}", "○".dimmed(), "不在 Git 仓库中".dimmed());
    }
    ...
    if let Some(ref hooks_path) = global_hooks_path {
        ...
        if hook_path.exists() {
            ...
            if is_gid {
                ...
                println!("  {} 全局 hook: {}", "!".yellow(), "已存在 (非 gid)".yellow());
            }
            println!("    {}", hook_path.display().to_string().dimmed());
        } else {
