...
fn configure_ssh_key(identity_id: &str, email: &str) -> Result<Option<PathBuf>> {
    ...
    let default_key = home.join(".ssh").join(format!("id_ed25519_{}", identity_id));
...
mod shellexpand {
    pub fn tilde(path: &str) -> std::borrow::Cow<str> {
        if path.starts_with("~/") {
            if let Some(home) = home::home_dir() {
                return std::borrow::Cow::Owned(format!("{}/{}", home.display(), &path[2..]));
            }
        }
