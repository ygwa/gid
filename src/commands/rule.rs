...
pub fn execute(action: RuleAction) -> Result<()> {
    match action {
        RuleAction::Add { rule_type, pattern, identity, priority } => {
        ...
        RuleAction::Test { path, remote } => {
            test_rule(path, remote)
        }
    }
}

/// 添加规则
fn add_rule(rule_type: CliRuleType, pattern: String, identity: String, priority: u32) -> Result<()> {
    let mut config = Config::load()?;
    
    // 验证身份存在
    if config.find_identity(&identity).is_none() {
        anyhow::bail!("身份 '{}' 不存在", identity);
    }
...
fn remove_rule(index: usize) -> Result<()> {
    ...
    if index >= config.rules.len() {
        anyhow::bail!("规则索引 {} 超出范围 (共 {} 条规则)", index, config.rules.len());
    }
